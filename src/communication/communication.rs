...
impl<T: ZellijService> Communication<T> {
    ...
    pub fn send_pipe_message(&self, envelope: &MessageEnvelope) -> Result<(), CommunicationError> {
        ...
        self.zellij_service
            .pipe_message_to_plugin(&payload, "coordination");
...
mod tests {
    ...
    fn test_send_pipe_message_with_different_envelope_types() {
        ...
        let broadcast_envelope =
            MessageEnvelope::new_broadcast(create_test_coordination_message(), "sender2");
    ...
    fn test_parse_incoming_message_all_coordination_variants() {
        ...
        for message in coordination_messages {
            ...
            let legacy_result =
                Communication::<MockZellijService>::parse_incoming_message(&legacy_json);
            ...
            let envelope_result =
                Communication::<MockZellijService>::parse_incoming_message(&envelope_json);
    ...
    fn test_edge_cases_with_special_characters() {
        ...
        let special_message = CoordinationMessage::StartPlanning {
            task_id: 999,
            task_description: "Task with special chars: !@#$%^&*()_+-=[]{}|;':\",./<>?`~"
                .to_string(),
        };
        ...
        let parse_result =
            Communication::<MockZellijService>::parse_incoming_message(&piped_messages[0].0);
        assert!(parse_result.is_ok());
        ...
        let unicode_parse =
            Communication::<MockZellijService>::parse_incoming_message(&updated_messages[1].0);
    ...
    fn test_send_pipe_message_with_multiple_calls() {
        ...
        let payloads: std::collections::HashSet<_> =
            piped_messages.iter().map(|(p, _)| p).collect();
    ...
    fn test_parse_incoming_message_empty_and_whitespace() {
        ...
        let whitespace_result =
            Communication::<MockZellijService>::parse_incoming_message("   \t\n  ");
