...
mod tests {
    ...
    fn test_message_delivery_failed_creation_and_display() {
        ...
        let display_message = format!("{}", error);
        assert_eq!(display_message, "Message delivery failed: Network timeout occurred");
    ...
    fn test_pane_discovery_failed_creation_and_display() {
        ...
        let display_message = format!("{}", error);
        assert_eq!(display_message, "Pane discovery failed: Failed to connect to Zellij API");
    ...
    fn test_from_serde_json_error_direct_conversion() {
        let result: Result<i32, CommunicationError> = serde_json::from_str("invalid").map_err(|e| e.into());
    ...
    fn test_error_display_edge_cases() {
        ...
        assert_eq!(format!("{}", empty_delivery_error), "Message delivery failed: ");
        assert_eq!(format!("{}", empty_target_error), "Invalid target pane: ");
        assert_eq!(format!("{}", empty_discovery_error), "Pane discovery failed: ");
        
        // Test with special characters
        let special_chars = "test-with-special_chars!@#$%^&*()";
        let special_delivery_error = CommunicationError::MessageDeliveryFailed(special_chars.to_string());
